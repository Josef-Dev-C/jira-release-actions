...
export class Project {
  ...
  async createVersion(version: Version): Promise<Version> {
    try {
      const response = await axios.post(
        `https://${this.domain}/rest/api/2/version`,
        version,
        this._authHeaders()
      )
  ...
  async updateVersion(version: Version): Promise<Version> {
    try {
      ...
      const response = await axios.put(
        `https://${this.domain}/rest/api/2/version/${version.id}`,
        version,
        this._authHeaders()
      )
      return response?.data
  ...
  async updateIssue(ticket: string, version: string) {
    try {
      const response = await axios.put(
        `https://${this.domain}/rest/api/2/issue/${ticket}`,
        {
          update: {
            fixVersions: [
              {
  ...
  async _load(): Promise<Project> {
    try {
      const response = await axios.get(
        `https://${this.domain}/rest/api/2/project/${this.name}?properties=versions,key,id,name`,
        this._authHeaders()
      )
